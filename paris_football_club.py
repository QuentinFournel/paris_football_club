...
def authenticate_google_drive():
    ...
    if creds and creds.expired and creds.refresh_token:
        try:
            ...
            creds = None  # Force la réauthentification

    # Si aucun token valide n'est trouvé, demande une nouvelle authentification
    ...
    if not files:
        ...
        for file in files:
            if file['name'].endswith('.csv') or file['name'].endswith('.xlsx'):  # Vérifier si le fichier est un .csv ou .xlsx
                ...
                print(f"Fichier ignoré (non .csv) : {file['name']}\n")
