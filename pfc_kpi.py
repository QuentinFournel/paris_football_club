...
def create_metrics(df):
    df['Timing'] = np.where(df['Duels défensifs'] > 0, (df['Duels défensifs'] - df['Fautes']) / (df['Duels défensifs'].sum() - df['Fautes'].sum()), 0)
    df['Force physique'] = np.where(df['Duels défensifs'] > 0, (df['Duels défensifs gagnés'] / df['Duels défensifs']) / (df['Duels défensifs gagnés'].sum() / df['Duels défensifs'].sum()), 0)
    ...
    df['Technique 2'] = np.where(df['Passes courtes'] > 0, (df['Passes réussies (courtes)'] / df['Passes courtes']) / (df['Passes réussies (courtes)'].sum() / df['Passes courtes'].sum()), 0)
    df['Technique 3'] = np.where(df['Passes longues'] > 0, (df['Passes réussies (longues)'] / df['Passes longues']) / (df['Passes réussies (longues)'].sum() / df['Passes longues'].sum()), 0)
    df['Explosivité'] = np.where(df['Dribbles'] > 0, (df['Dribbles réussis'] / df['Dribbles']) / (df['Dribbles réussis'].sum() / df['Dribbles'].sum()), 0)
    ...
    df['Précision'] = np.where(df['Tirs'] > 0, (df['Tirs cadrés'] / df['Tirs']) / (df['Tirs cadrés'].sum() / df['Tirs'].sum()), 0)
