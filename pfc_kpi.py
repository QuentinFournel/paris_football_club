...
@st.cache_data
def collect_data(upload_files):
    pfc_kpi = pd.DataFrame()

    for upload_file in upload_files:

        if upload_file.name.endswith('.xlsx'):
            print(f'\'{upload_file.name}\' : Récupération des statistiques en cours...')
            
            # Charger un fichier Excel
            edf = pd.read_excel(upload_file)
            ...
            match_1 = pd.read_csv(io.BytesIO(next(file for file in upload_files if file.name == "EDF_U19_Match1.csv").read()))
            match_2 = pd.read_csv(io.BytesIO(next(file for file in upload_files if file.name == "EDF_U19_Match2.csv").read()))
            match_3 = pd.read_csv(io.BytesIO(next(file for file in upload_files if file.name == "EDF_U19_Match3.csv").read()))
            ...
            print(f'\'{upload_file.name}\' : Récupération des statistiques terminée.\n')
            ...
            if 'PFC' not in upload_file.name:
                continue
            
            # Charger un fichier CSV
            data = pd.read_csv(io.BytesIO(upload_file.read()))

            # On supprime l'extension du fichier
            base_filename = upload_file.name.split('.')[0]
            print(base_filename)
            ...
            if len(joueurs) > 0:
                ...
                print(f'\'{upload_file.name}\' : Aucune statistique à récupérer.\n')
...
def script_streamlit():
    ...
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader("Téléchargez un ou plusieurs fichiers", type=["csv", "xlsx"], accept_multiple_files=True)
        if uploaded_files:
            st.success(f"{len(uploaded_files)} fichier(s) chargé(s) avec succès !")

            if st.button("Collecter les données"):
                if len(uploaded_files) >= 5 and sum('EDF' in file.name for file in uploaded_files) >= 4 and sum('PFC' in file.name for file in uploaded_files) >= 1:
                    st.session_state.pfc_kpi, st.session_state.edf_kpi = collect_data(uploaded_files)
                else:
                    st.warning(
                        "Veuillez charger au moins 5 fichiers :\n"
                        "- Un fichier de match du PFC\n"
                        "- EDF_Joueuses.xlsx\n"
                        "- EDF_U19_Match1.csv\n"
                        "- EDF_U19_Match2.csv\n"
                        "- EDF_U19_Match3.csv"
            ...
            st.warning("Veuillez télécharger un ou plusieurs fichiers.")
            page = "Données"

    # Vérification si les données ont été collectées
    if 'pfc_kpi' not in st.session_state or 'edf_kpi' not in st.session_state:
        st.write(
            """
            ⚠ **Consignes pour les fichiers :**  
            
            - Le nom des fichiers hors EDF doit respecter ce format :  
            **"Équipe à domicile_VS_Équipe à l'extérieur_Journée_Catégorie_DD-MM-YYYY.csv"**  
            **Exemple** : `PFC_VS_FC 93_J7_U18_14-12-2024.csv`
            
            - **Attention au nom des équipes**  
            Le nom des équipes dans le nom du fichier doit être **exactement** le même que dans la colonne "Row" des données collectives (y compris les espaces et les caractères spéciaux).
            
            - Les fichiers hors EDF doivent contenir à la fois des **données collectives et individuelles**, respectant le format habituel (sinon, il faut modifier le code pour accepter un autre format).
            """
        )
        return
    
    # Utilisation des données collectées dans la session
